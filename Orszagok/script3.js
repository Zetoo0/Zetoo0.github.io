...
function check() {
    ...
    let countryName = document.getElementById("bevitel").value;
    if (datas0[randomroll].name == countryName) {
        summ.innerHTML = '<h4>Gratulálok, a helyes megoldás: ' + datas0[randomroll].name + '</h4>';
        goodAnswersNum++;
        goodAnswersDiv.innerHTML = "Eltalált városok száma: " + goodAnswersNum;
        badAnswersDiv.innerHTML = "Rossz válaszok száma: " + badAnswersNum;
        correctAudio.play();
    } else {
        summ.innerHTML = '<h4>Nem jó, a helyes megoldás: ' + datas0[randomroll].name + '</h4>';
        badAnswersNum++;
        goodAnswersDiv.innerHTML = "Eltalált városok száma: " + goodAnswersNum;
        badAnswersDiv.innerHTML = "Rossz válaszok száma: " + badAnswersNum;
        incorrectAudio.play();
